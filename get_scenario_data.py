...
# story:
# here's how well the RPS policy will do vs. a fossil/bau baseline or vs. a least-cost plan
# (not clear which would be the real baseline); generally superior to bau baseline,
...
scenarios = (
    ...
    [
        # standard settings with various price levels and policies
        (
            '{}_oil_{}_tech_{}'.format(oil_price, tech_price, policy),
            '--inputs-dir inputs/{}_oil_{}_tech_{} '
            .format(oil_price, tech_price, 'bau' if policy == 'bau' else 'rps')
            + (' --rps-deactivate' if policy == 'uncons' else '')
            + (
                ' --rps-no-new-renewables --no-hydrogen'
                ' --demand-response-share 0.0 --ev-timing bau'
                ' --exclude-module switch_model.hawaii.pumped_hydro'
                if policy == 'bau' else ''
            )
            + (' --rps-targets 2020 0.4 2025 0.7 2030 1.0' if policy == '2030' else '')
        )
        ...
        for policy in ['2045', 'bau', '2030']  # , 'uncons']
    ] + [
        ...
        ('lng_2020_2045', '--force-lng-tier container_25 2020 2044 --inputs-dir inputs/mid_oil_mid_tech_rps'),
        ('lng_2025_2045', '--force-lng-tier container_20 2025 2044 --inputs-dir inputs/mid_oil_mid_tech_rps'),
        ('lng_2020_2030', '--force-lng-tier container_10 2020 2029 --rps-targets 2020 0.4 2025 0.7 2030 1.0 --inputs-dir inputs/mid_oil_mid_tech_rps'),
        ('lng_2025_2030', '--force-lng-tier container_05 2025 2029 --rps-targets 2020 0.4 2025 0.7 2030 1.0 --inputs-dir inputs/mid_oil_mid_tech_rps'),

        # siting/technology challenges (all using reference fuel and tech prices)
        # no more wind than is already in place
        ('no_new_wind', '--rps-no-new-wind --inputs-dir inputs/mid_oil_mid_tech_rps'),
        # no hydrogen
        ('no_hydrogen', '--no-hydrogen --inputs-dir inputs/mid_oil_mid_tech_rps'),
        ...
        (
            ...
            ' --inputs-dir inputs/mid_oil_mid_tech_rps'
        ),
        # no demand response, business-as-usual EV charging
        ('dr_0', '--demand-response-share 0.0 --ev-timing bau --inputs-dir inputs/mid_oil_mid_tech_rps'),
        # three EV charging scenarios
        # no demand response, business-as-usual EV charging
        ('dr_0_ev_bau', '--demand-response-share 0.0 --ev-timing bau --inputs-dir inputs/mid_oil_mid_tech_rps'),
        # no DR, optimal EV charging
        ('dr_0_ev_opt',  '--demand-response-share 0.0 --inputs-dir inputs/mid_oil_mid_tech_rps'),
        # 10% DR, bau EV charging
        ('dr_10_ev_bau', '--ev-timing bau --inputs-dir inputs/mid_oil_mid_tech_rps'),
    ]
)
# show range of cost for 2045 RPS, 2030 RPS or bau, and show range of
# savings or costs for RPS vs. bau in each economic environment
...
# args['aliases'] = {
#     'bau_ev_share': dict(ev_scenario='Business as Usual')
# }

oil_price_tags = {'low': 'Low_Oil_Prices', 'mid': 'Reference', 'high': 'High_Oil_Prices'}
...
alt_args = [
    ...
    dict(
        [
            ('inputs_subdir', '{}_oil_{}_tech_{}'.format(oil_price, tech_price, pol)),
            ('fuel_scen_id', 'AEO_2018_{}'.format(oil_price_tags[oil_price])),
            ('tech_scen_id', 'ATB_2018_{}'.format(tech_price)),
        ] + ([
            ('ev_scenario', 'Business as Usual'),
            (
                'exclude_technologies',
                args.get('exclude_technologies', '')
                + ('Battery_Bulk', 'Battery_Conting', 'Battery_Reg')
            )
        ] if pol == 'bau' else [])
    )
    for oil_price in ['mid', 'low', 'high']
